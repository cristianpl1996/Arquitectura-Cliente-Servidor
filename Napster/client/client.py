	...
	def getplaylist(self):
		...
		for archivo in os.listdir(self.dir):
			...
			with open(os.path.join(self.dir,archivo), 'rb') as file:
				song = file.read()
			self.PLAYLIST.append({"Address": myAddress,
...
def main():
	...
	def search():
		...
		search = txt.get()
		if len(search) == 0:
			messagebox.showerror('Información ©', 'Digite busqueda a realizar.')
		else:
			filter = combo.get()
			if (filter == 'Cancion'):
			...
			try:
				...
				if ((name == 0) and (artist == 0) and (album == 0) and (size == 0) and (address == 0)):
					messagebox.showerror('Información ©', 'No se han encontrado resultados para tu búsqueda(' + search + ').')
				else:
					for x in address:
						if (x == myAddress):
							address.remove(x)
					...
					if (len(address) != 0):
						...
						messagebox.showerror('Información ©', 'No hay descarga.')	
					txt.delete(0, END)
			except zerorpc.exceptions.LostRemote as e:
				addressServer = addressServers[1]
				messagebox.showerror('Información ©', 'Tuvimos un error intentalo de nuevo.')
